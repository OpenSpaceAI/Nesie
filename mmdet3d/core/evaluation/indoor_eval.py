...
def indoor_eval(gt_annos,
    ...
    for img_id in range(len(dt_annos)):
        ...
        # for i in range(len(det_anno['pseudo_label'])):
        #     label = det_anno['pseudo_label'].numpy()[i]
        #     bbox = det_anno['boxes_3d'].convert_to(box_mode_3d)[0]
        #     bbox.tensor[0] = det_anno['pseudo_boxes'][i]
        #     # bbox.tensor[0, 2] = bbox.tensor[0, 2] + bbox.tensor[0, 4] * 0.5
        #     score = det_anno['pseudo_var_weight'].numpy()[i]
        #     score = 1.0
