...
class VoteNetSAQE(SingleStage3DDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                custom_config=None,):
        super(VoteNetSAQE, self).__init__(
            backbone=backbone,
            bbox_head=bbox_head,
            train_cfg=train_cfg,
            test_cfg=test_cfg,
    ...
    def forward_train(self, label_data, unlabel_datas, img_metas):
        ...
        sup_losses = self.bbox_head.sup_loss(sup_bbox_preds, *sup_loss_inputs)
    ...
    def get_pseudo_labels(self, unsup_bbox_preds, dataset_name="ScanNet"):
        ...
        # obj score threshold
        pred_objectness = torch.nn.Softmax(dim=2)(unsup_bbox_preds['R_obj_scores'])
        ...
        quality_score = 0.8* side_scores * side_scores - 1.8* side_scores + torch.ones_like(side_scores)
        ...
        if self.test_cfg.add_info:
            ...
            bbox_results[0] = dict(
                **bbox_results[0], 
                ...
                obj_scores = bbox_preds['R_obj_scores'].to('cpu'),
