...
class SidePooling(nn.Module):
    def __init__(
        ...
        # side 
        for _ in range(6):
            self.mlps_before.append(MiniPointNet(self.seed_feat_dim + 3, 128))
    ...
    def grid_for_side(
        ...
        side_grid = torch.cat(
            [front_grid, back_grid, top_grid, down_grid, left_grid, right_grid], dim=-2
        )
    ...
    def forward(
        ...
        side_interpolated_feats = torch.split(
            side_interpolated_feats, self.grid_size * self.grid_size, dim=-1
        )
        bbox_interpolated_feats = self.grid_features(
            origin_xyz, origin_features, bbox_grid, center
        )
        ...
        bbox_feats = self.mlps_before[6](bbox_interpolated_feats)
        end_points[f"{prefix}iou_scores"] = self.mlps_head[6](bbox_feats).transpose(2, 1)
        return end_points
